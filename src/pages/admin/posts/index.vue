...
watch(
    ()=>postStore.posts,
    (newPosts)=>{
      countPosts.value = newPosts?.length ?? 0
    },
    {
      deep: true,
  ...
  <div>
    <div class="mb-1 flex justify-between">
      <div class="flex my-auto mx-auto ">
        <h3 class="text-lg font-bold uppercase"> all Posts</h3>
        <span class="ml-2 h-full my-auto">(count: {{ countPosts }})</span>
      </div>
      <div>
    ...
    <div>
      <table class="w-full border border-gray-200">
        ...
        <tbody>
        <tr v-for="post in postStore.posts" class="bg-white border-b border-gray-200 text-left p-2">
          <td>{{ post.id }}</td>
          <td>
          ...
          <td>
            <div class="flex mx-auto">
              <div class="mr-2">
                <svg @click="postStore.deletePost(post.id)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
